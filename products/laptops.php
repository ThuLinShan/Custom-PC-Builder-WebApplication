...
if (isset($_GET['category'])) {
    ...
    $query = "SELECT * FROM laptop WHERE category = $category ORDER BY date LIMIT $skip,$size";
    $laptops = mysqli_query($connection, $query);
    $query = "SELECT COUNT(*) AS count FROM laptop WHERE category = $category";
} else {
...
<main>
    ...
    <div class="container my-5">
        ...
        <?php if ($count <= 0) : ?>
            <div class="py-5">
                <h5 class="text-center bg-info p-3 rounded text-white">
                    Currently, there is no laptops for this category.
                </h5>
            </div>
        <?php else : ?>
                ...
                <?php while ($laptop = mysqli_fetch_assoc($laptops)) : ?>
                    <div class="col-xl-3 col-lg-4 col-md-6 col-sm-12 mb-3 py-4 rounded">
                        <div class="card mx-auto" style="width: 18rem;">
                            ...
                            <img src="<?= ROOT_URL . "assets/images/products/laptop/" . $laptop['img'] ?>" class="card-img-top" alt="..." style="height: 200px; object-fit: cover;">
                            ...
                            <div class="card-body">
                                ...
                                <a href="<?= ROOT_URL ?>authenticated/add_to_cart_logic.php?product_id=<?= $laptop['id'] ?>&category=laptop&count=1&price=<?= $laptop['price'] ?>" class="btn rounded-5 btn-secondary">Add to Cart</a>
                            </div>
                        </div>
                    </div>
                <?php endwhile; ?>
